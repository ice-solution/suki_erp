...
export default function InvoiceOrderNumberFields({
  ...
  const cachedSuggested = getSuggestedNextInvoiceNumber(mergedLastNumbers, prefix);
  const [loading, setLoading] = useState(false);

  const applyLiveSuggested = async () => {
    try {
      setLoading(true);
      const next = await fetchSuggestedNextNumber('invoice', prefix);
      if (next == null) {
  ...
  return (
    <div style={{ marginBottom: 16, padding: 12, background: '#fafafa', borderRadius: 6 }}>
      ...
      <p style={{ color: '#888', fontSize: 12, margin: '8px 0 0' }}>
        按一下向伺服器查詢目前最後號碼 +1。須大於最後號碼，且不可與現有發票重複。
      </p>
    </div>
  );
}
