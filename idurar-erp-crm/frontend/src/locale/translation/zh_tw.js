const lang = {
  sign_up: '註冊',
  manage_your_company_with: '管理您的公司',
  all_in_one_tool: '一體化工具',
  run_and_scale_your_erp_crm_apps: '運行和擴展您的ERP CRM應用',
  easily_add_and_manage_your_services: '輕鬆添加和管理您的服務',
  it_brings_together_your_invoice_clients_and_leads: '它將您的發票、客戶和潛在客戶整合在一起',
  select_language: '選擇語言',
  register: '註冊',
  or: '或',
  already_have_account_login: '已有帳戶登入',
  name: '姓名',
  email: '電郵',
  password: '密碼',
  country: '國家',
  dashboard: '儀表板',
  customer: '客戶',
  invoice: '發票',
  quote_number: '報價單號',
  quote: '報價單',
  quotation: '報價單',
  quotation_list: '報價單列表',
  add_new_quotation: '新增報價單',
  payment: '付款',
  payment_mode: '付款方式',
  taxes: '稅項',
  settings: '設定',
  about: '關於',
  customers: '客戶',
  invoices: '發票',
  quotes: '報價單',
  payments: '付款',
  payments_mode: '付款方式',
  contractor: '承辦商',
  contractors: '承辦商',
  contractor_employee: '承辦商員工',
  contractor_employees: '承辦商員工',
  project: '項目',
  projects: '項目',
  project_items: '工程項目',
  contractor_management: '承辦商管理',
  project_management: '項目管理',
  search: '搜尋',
  supplier_quote: 'S單',
  ship_quote: '吊船報價',
  warehouse_management: '存倉管理',
  project_report: '項目報告',
  ship_management: '船隻管理',
  winch_management: '爬纜器管理',
  registrationnumber: '登記號',
  member_invoice_stats: '會員發票統計',
  profile_settings: '個人設定',
  app_settings: '應用設定',
  logout: '登出',
  number: '編號',
  client: '客戶',
  client_list: '客戶列表',
  add_new_client: '新增客戶',
  supplier: '供應商',
  supplier_list: '供應商列表',
  add_new_supplier: '新增供應商',
  suppliers: '供應商',
  total: '總計',
  status: '狀態',
  close: '關閉',
  save: '儲存',
  edit: '編輯',
  delete: '刪除',
  cancel: '取消',
  add_new: '新增',
  show: '顯示',
  download: '下載',
  download_pdf: '下載 PDF',
  preview_pdf: '預覽 PDF',
  send_by_email: '電郵發送',
  convert_to_invoice: '轉換成發票',
  this_month: '本月',
  not_paid: '未付款',
  paid: '已付款',
  unpaid: '未付',
  recent_invoices: '最近發票',
  recent_quotes: '最近報價',
  draft: '草稿',
  pending: '待處理',
  sent: '已發送',
  cancelled: '已取消',
  accepted: '已接受',
  declined: '已拒絕',
  expired: '已過期',
  overdue: '逾期',
  partially: '部分付款',
  sub_total: '小計',
  subtotal: '小計',
  discount: '折扣',
  amount: '金額',
  item: '項目',
  description: '描述',
  quantity: '數量',
  price: '單價',
  address: '地址',
  date: '日期',
  year: '年',
  type: '類型',
  expire_date: '到期日',
  expired_date: '到期日',
  contact_person: '聯絡人',
  invoice_number: '發票編號',
  payment_status: '付款狀態',
  language: '語言',
  clients: '客戶',
  clients_information: '客戶資料',
  quote_details: '報價詳情',
  product: '產品',
  payment_information: '付款資料',
  record_payment: '記錄付款',
  reference: '參考',
  note: '備註',
  notes: '備註',
  created_by: '建立者',
  update: '更新',
  refresh: '重新整理',
  remove: '移除',
  submit: '提交',
  back: '返回',
  default: '預設',
  value: '數值',
  currency: '貨幣',
  general_settings: '一般設定',
  company_settings: '公司設定',
  finance_settings: '財務設定',
  money_format_settings: '金額格式設定',
  contact_list: '聯絡人',
  add_contact: '新增聯絡人',
  phone: '電話',
  project_percentage: '整個佔比 (%)',
  project_percentage_short: '整個佔比%',
  project_percentage_hint: '自動計算：本張發票總額 ÷ Project 總額 × 100（小數點後 2 位，四捨五入）',
  paid_date: '已付款日期',
  prepared_by: '跟單人',
  modified_by: '修改人',
  last_invoice_number: '發票最後編號',
  last_smi_number: 'SMI 最後編號',
  last_wse_number: 'WSE 最後編號',
  last_sp_number: 'SP 最後編號',
  last_quote_number: '報價單最後編號',
  last_sml_number: 'SML 最後編號',
  last_qu_number: 'QU 最後編號',
  default_quote_supplier_name: '報價預設供應商名稱',
  default_quote_supplier_id: '報價預設供應商',
  default_quote_supplier_id_hint: '建立報價單時，若未選供應商則自動帶入此商戶',
  quote_numbers_section: '報價單最後編號',
  invoice_numbers_section: '發票最後編號',
  supplier_numbers_section: 'S 單最後編號',
  quote_default_supplier_section: '報價預設供應商',
  select_supplier: '請選擇供應商',
  supplier_last_number_no: 'Supplier 編號 (NO) 最後號碼',
  supplier_last_number_po: 'Supplier 編號 (PO) 最後號碼',
  supplier_last_number_s: 'Supplier 編號 (S) 最後號碼',
  supplier_last_number_swp: 'Supplier 編號 (SWP) 最後號碼',
  supplier_last_number_e: 'Supplier 編號 (E) 最後號碼',
  supplier_last_number_y: 'Supplier 編號 (Y) 最後號碼',
  supplier_last_number_ip: 'Supplier 編號 (IP) 最後號碼',
  supplier_last_number_ih: 'Supplier 編號 (IH) 最後號碼',
};

export default lang; 