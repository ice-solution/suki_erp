...
export const DEFAULT_SHIP_RENTAL_EXTRA_ITEMS = [
  ...
  { description: '三角足場續租(每14日)', unit: '部', unitPrice: 1000 },
  { description: '三角足場裝設及拆卸費用', unit: '部', unitPrice: 5000 },
];
...
function LoadShipQuoteTableForm({ subTotal: propSubTotal = 0, current = null }) {
  ...
  const handleDiscountAmountChange = (value) => {
    ...
    if (value == null || value === '') {
      ...
      setDiscountTotal(0);
      setTotal(Number.parseFloat(Number(subTotal).toFixed(2)));
      return;
    }
    ...
    setTotal(Number.parseFloat(Number(calculate.sub(subTotal, amount)).toFixed(2)));
  };
