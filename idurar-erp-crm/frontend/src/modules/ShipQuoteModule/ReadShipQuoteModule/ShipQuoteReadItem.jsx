...
import { Button, Row, Col, Descriptions, Statistic, Tag, Modal, message, Table, Space, Input, Select, InputNumber, Radio } from 'antd';
...
export default function ShipQuoteReadItem({ config, selectedItem }) {
  ...
  const executeConvertToInvoice = async () => {
    ...
    try {
      ...
      const response = await axios.post(`shipquote/convert/${currentErp._id}`, payload);
  ...
  return (
    <>
      ...
      >
        ...
        {selectedPoNumber && poModalMode === 'invoice' && invoiceConversionMode === 'B' ? (
          <p style={{ color: '#1890ff', fontSize: '12px', marginTop: 12 }}>
            ℹ️ B 模式（逐項專案佔比）：每行填寫本次轉出 %；發票該行金額 = <strong>項目金額 × 佔比%</strong>
