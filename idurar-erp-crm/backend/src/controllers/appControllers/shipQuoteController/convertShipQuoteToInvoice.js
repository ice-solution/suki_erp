...
/**
 * 將 ShipQuote 轉換為 Invoice（吊船 quote -> invoice）
 * 與 Quote 轉換一致：可重複轉換；可選 itemIndices（逗號分隔 0-based），不傳則全部項目。
 */
const convertShipQuoteToInvoice = async (req, res) => {
  try {
    ...
    let selectedItems = shipQuote.items;
    ...
    if (itemIndicesParam && typeof itemIndicesParam === 'string' && itemIndicesParam.trim() !== '') {
      const indices = itemIndicesParam
        ...
        .filter((n) => !Number.isNaN(n) && n >= 0 && n < shipQuote.items.length);
      if (indices.length === 0) {
        return res.status(400).json({
          success: false,
          result: null,
      ...
      selectedItems = indices.map((i) => shipQuote.items[i]);
    }
    ...
    const invoiceData = {
      ...
      poNumber: resolveInvoicePoNumberForConversion(shipQuote, selectedItems),
