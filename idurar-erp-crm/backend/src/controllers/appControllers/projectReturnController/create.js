...
const create = async (req, res) => {
  try {
    ...
    // 創建退回記錄
    const returnRecord = new ProjectReturn({
      project,
      returnNumber,
    ...
    return res.status(201).json({
      success: true,
      result: returnRecord,
      message: 'Project return record created successfully',
    });
    ...
    return res.status(500).json({
      success: false,
      result: null,
      message: 'Error creating return record: ' + error.message,
    });
  }
};
...
async function generateReturnNumber() {
  ...
  // 查找當天的最大序號
  const lastRecord = await ProjectReturn.findOne({
    returnNumber: { $regex: `^${prefix}` }
  }).sort({ returnNumber: -1 });

  let sequence = 1;
  if (lastRecord) {
    const lastSequence = parseInt(lastRecord.returnNumber.split('-')[1].slice(-3));
