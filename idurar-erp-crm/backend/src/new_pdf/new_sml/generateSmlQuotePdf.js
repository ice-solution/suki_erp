/**
 * 使用 Puppeteer（Headless Chromium）將 sml.pug 渲染為 PDF。
 * 與 html-pdf 並存，供測試版面與列印效果。
 *
 * @module new_pdf/new_sml/generateSmlQuotePdf
 */
...
function resolveLogoSettingKeyForQuote(result) {
  if (result && result.numberPrefix === 'SML') {
    return 'company_logo_sml';
  }
  return null;
}

function resolveSmlTemplatePath() {
  const fileNameLower = 'sml.pug';
  const candidates = [
  ...
  if (!found) {
    throw new Error(`sml.pug not found. Tried: ${candidates.join(', ')}`);
  }
  return found;
}
 ...
 */
async function generateSmlQuotePdfBuffer(model) {
  ...
  const templatePath = resolveSmlTemplatePath();
