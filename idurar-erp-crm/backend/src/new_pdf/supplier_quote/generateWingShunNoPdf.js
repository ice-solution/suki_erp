/**
 * 永順 NO 發收單：Puppeteer + no.pug + 標準文字頁尾（與 QU/WSE 一致，每頁貼底）
 */
const fs = require('fs');
...
function resolveNoTemplatePath() {
  ...
  if (!found) {
    throw new Error(`no.pug not found. Tried: ${candidates.join(', ')}`);
  }
  return found;
}

async function generateWingShunNoPdfBuffer(model) {
  ...
  const htmlContent = pug.renderFile(resolveNoTemplatePath(), {
    model,
    settings,
    translate,
    dateFormat,
